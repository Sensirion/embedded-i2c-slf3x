# Sensirion I2C SLF3X embedded Library

This document explains how to set up a sensor of the SLF3X family to run on an embedded device using the I²C interface.
...
Your sensor has 4 pins that need to be connected to your board: SDA, VDD, GND, SCL.
Use the following description to connect your SLF3X:
...
In order to use the code with another product or I²C address you need to change it in the call slf3x_init(ADDRESS) in
`slf3x_i2c_example_usage.c`. The list of supported I²C-addresses is found in the header
`slf3x_i2c.h`.
...
5. Run the compiled executable with `./slf3x_i2c_example_usage`
...
5. Run the compiled executable with `./slf3x_test_hw_i2c`.
